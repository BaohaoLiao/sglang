...
"""Inference-only SDAR models compatible with HuggingFace weights."""
...
def _get_sliding_window_size(config) -> Optional[int]:
...
class SDARDecoderLayer(nn.Module):
    def __init__(
        ...
        sliding_window = _get_sliding_window_size(config)
...
class SDARModel(Qwen2Model):
    def __init__(
        ...
        super().__init__(
            ...
            quant_config=quant_config,
            prefix=prefix,
            decoder_layer_type=SDARDecoderLayer,
            alt_stream=alt_stream,
        )

    def get_attention_sliding_window_size(self):
        return _get_sliding_window_size(self.config)
...
def _is_sparse_layer(config, layer_id: int) -> bool:
    decoder_sparse_step = getattr(config, "decoder_sparse_step", None)
    if decoder_sparse_step is None or decoder_sparse_step <= 0:
        return False
    if getattr(config, "num_experts", 0) <= 0:
        return False
    mlp_only_layers = set(getattr(config, "mlp_only_layers", []))
    return (layer_id + 1) % decoder_sparse_step == 0 and (
...
class SDARMoeSparseMoeBlock(nn.Module):
    ...
    def forward(
        ...
        return final_hidden_states.view(batch_size, sequence_length, -1)
...
class SDARMoeDecoderLayer(nn.Module):
    def __init__(
        ...
        self.is_layer_sparse = _is_sparse_layer(config, layer_id)
        is_previous_layer_sparse = _is_sparse_layer(config, layer_id - 1)
    ...
    def op_mlp(self, state):
        ...
        if self.is_layer_sparse:
            ...
            state.hidden_states_mlp_output = self.mlp(hidden_states)
...
class SDARMoeModel(Qwen2MoeModel):
    def __init__(
        self,
        config,
        quant_config: Optional[QuantizationConfig] = None,
        prefix: str = "",
        decoder_layer_type=SDARMoeDecoderLayer,
    ) -> None:
        alt_stream = torch.cuda.Stream() if _is_cuda else None
        super().__init__(
            config=config,
    ...
    @staticmethod
    def get_model_config_for_expert_location(config):
...
EntryClass = [SDARForCausalLM, SDARMoeForCausalLM]
